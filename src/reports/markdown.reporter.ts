...
export class MarkdownReporter {
  ...
  private generateExecutiveSummary(findings: Finding[]): string {
    ...
    if (critical > 0) {
      lines.push(
        `**⚠️ CRITICAL:** ${critical} critical vulnerabilit${critical === 1 ? "y" : "ies"} found requiring immediate attention.`
      );
    }

    if (high > 0) {
      lines.push(
        `**🔴 HIGH:** ${high} high severity issue${high === 1 ? "" : "s"} should be addressed promptly.`
      );
    }
